...
class TestFixMode:
    ...
    def test_a_fixup_that_landed_partly_is_named_in_the_row_and_the_log(
        ...
        The door is the real one: `spot_to_process` cannot describe a colorant
        inside a shading that maps a point in the plane, so it converts every
        other occurrence and leaves that one live. The document is its own
        one-file tree because the shared `_tree` is pinned at three documents
        by the counts every other test here asserts.
        """
        source = tmp_path / "in"
        source.mkdir()
        builders.build("spot_gradient_unconvertible", str(source), name="gradient")
        report = run_preflight_sweep(
            str(source), str(tmp_path / "out"),
            profile=_profile(
        ...
        row = next(r for r in report["results"] if r["rel"] == "gradient.pdf")
        assert row["applied"] == ["spots_to_process"]
        assert [entry["fixup"] for entry in row["partial"]] == ["spots_to_process"]
        assert [i["item"] for i in row["partial"][0]["partial"]] == ["GradientSpot"]
        log = pathlib.Path(report["log_path"]).read_text(encoding="utf8")
        assert "spots_to_process left: GradientSpot" in log
