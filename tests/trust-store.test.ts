...
describe('trust configuration', () => {
  ...
  it('persists anchors and the store opt-in independently', () => {
    ...
    expect(loadTrustConfig()).toEqual({ anchors: ['C:/ca.pem'], systemStore: false });
    saveSystemStore(true);
    expect(loadTrustConfig()).toEqual({ anchors: ['C:/ca.pem'], systemStore: true });
    saveTrustAnchors([]);
    expect(loadTrustConfig()).toEqual({ anchors: [], systemStore: true });
  });
...
describe('engine params', () => {
  it('sends NO system_trust key while the option is off', () => {
    ...
    expect(trustVerifyParams({ anchors: [], systemStore: false })).toEqual({});
    expect(trustVerifyParams({ anchors: ['C:/ca.pem'], systemStore: false })).toEqual({
      trust_roots: ['C:/ca.pem'],
    });
  });

  it('sends the opt-in with or without anchors', () => {
    expect(trustVerifyParams({ anchors: [], systemStore: true })).toEqual({ system_trust: true });
    expect(trustVerifyParams({ anchors: ['C:/ca.pem'], systemStore: true })).toEqual({
      trust_roots: ['C:/ca.pem'],
      system_trust: true,
    });
  });
...
describe('aggregate trust readout', () => {
  const anchorsOnly: TrustConfig = { anchors: ['C:/ca.pem'], systemStore: false };
  const storeOnly: TrustConfig = { anchors: [], systemStore: true };
  const both: TrustConfig = { anchors: ['C:/ca.pem'], systemStore: true };

  it('separates "no source configured" from "did not verify"', () => {
    const trusted = result([sig({ trusted: true, trust_source: 'user' })]);
    expect(trustSummary({ anchors: [], systemStore: false }, trusted)).toBe('none');
