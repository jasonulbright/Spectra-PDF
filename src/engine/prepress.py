...
from .acroform import has_form_fields, reattach_forms_file
...
def _stage_carve_out(source: Path, scratch: Path, annotations: bool,
    ...
    with pikepdf.open(str(source)) as pdf:
        ...
        boxes = _stage_form_appearances(pdf) if forms else []
...
# ── the form's own appearances ─────────────────────────────────────────────
#
# The producer drops every widget annotation and FLATTENS its appearance into
# the page content (measured), while the field reattach that follows puts the
# widget back wearing the appearance the ORIGINAL had. That cost the document
# two things at once: the field was painted TWICE — once as page content, once
# by the widget — and the surviving `/AP` was never colour-converted, so an RGB
# paint inside a widget appearance came out of "Convert to CMYK" still RGB.
#
# No producer switch reaches it. `-dPreserveAnnots` changes nothing for a
# widget (dropped and flattened under either value, measured), and
# `-dShowAnnots=false` suppresses EVERY annotation's appearance — which would
# cost a dropped `/PrinterMark` the plate it only keeps because its appearance
# was flattened into the page.
#
# So the widget is taken out of the producer's input, leaving nothing to
# flatten, and each appearance FACE is converted on a staged page of its own
# whose box is the face's own. The producer converts a page exactly as it
# converts a flattened appearance (measured identical, plate for plate), the
# carve-out reaches a staged page as ordinary page content, and EVERY face is
# converted rather than only the one `/AS` selects. The converted pages are
# harvested back onto the faces, and the fields reattach from that file.
#
# Two boundaries, both measured:
#   - a widget with NO appearance stays where it is. The producer SYNTHESIZES
#     one from `/V` and `/DA` and flattens that, which is already a single
#     converted painter; removing the widget would erase it.
#   - the harvest runs BEFORE `_rebase_appearances`, which must never see a
#     harvested face: that face's content came from a page, so the pattern
...
def _face_streams(appearance) -> list:
...
def _staged_faces(pdf) -> list:
    """The widget appearance faces that paint, in the order that names them.

    The same file walked again yields the same list, which is how a converted
    ...
    appearance is staged once and converted once.
    """
    seen: set = set()
    out: list = []
    for page in pdf.pages:
        for annot in list(page.obj.get("/Annots") or []):
            ...
            if appearance is None:
                continue
            for face in _face_streams(appearance):
...
def _stage_form_appearances(pdf) -> list:
...
def _staged_matrix(staged, produced):
    ...
    pdfwrite moves a page's origin to (0, 0) (measured), so the converted
...
def _convert_form_appearances(output: Path, source: Path, scratch: Path,
    ...
    costs the appearance its conversion and never the document its fields. The
    staged pages come off either way: they are scaffolding, and a document that
    shipped them would carry an extra page per appearance face.
    """
    if not boxes:
        return None
    forms = scratch / "forms.pdf"
...
def convert_cmyk(
    ...
    try:
        ...
        rasterized = _after_restore(output_path, input_path, claimed, rasterized,
                                    annotations=True)
        forms_source = _convert_form_appearances(output_path, input_path,
